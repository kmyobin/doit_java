...
public class StringClassTest {
	...
	public static void main(String[] args) throws ClassNotFoundException{
		...
		Class perClass=Class.forName("classex.Person");

		Constructor[] cons=perClass.getConstructors(); // 모든 생성자 가져오기
		for(Constructor c:cons) {
			System.out.println(c);
		}
		
		System.out.println();
		Field[] fields=perClass.getFields(); // 모든 멤버 변수(필드) 가져오기
		for(Field f:fields) {
			System.out.println(f);
		}
		System.out.println();
		Method[] methods=perClass.getMethods(); // 모든 메서드 가져오기
		for(Method m:methods) {
			System.out.println(m);
		}
